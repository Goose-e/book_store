...
SUDO_PASS="Kolos213" # root user password, super user, then delete for security
...
DB_URL="jdbc:postgresql://$DB_HOST_NAME:$DB_PORT/$DB_NAME?user=$DB_USER_ROLE&password=$DB_PASSWORD" # for datagrip
echo $DB_URL > ~/.lastDbUrl

DB_R2DBC_URL="r2dbc:pool:postgresql://$DB_HOST_NAME:$DB_PORT/$DB_NAME" # in application.yml as name bd
